...
export default function TheList() {
  ...
  const [schedules, setSchedules] = useState(function() { return loadFromStorage("tl_schedules", {}); });
  ...
  const getMonthDays = function() {
    ...
    for (var j=1;j<=daysInMonth;j++) days.push(new Date(baseDate.getFullYear(),baseDate.getMonth(),j));
    return days;
  };
  ...
  const undoHistoryEntry = function(entry) {
    ...
    if (entry.type === "removed") {
      ...
      var idx1 = slots1.findIndex(function(s){ return s.time===entry.time; });
      if (idx1>=0 && slots1[idx1].name && slots1[idx1].name!==entry.name) {
        if (!window.confirm("⚠ "+slots1[idx1].name+" is now in that slot. Undoing will displace them. Continue?")) return;
      }
    }
    if (entry.type === "added") {
      var slots2 = [...getSlots(entry.dateKey)];
      var idx2 = slots2.findIndex(function(s){ return s.time===entry.time&&s.name===entry.name; });
      if (idx2>=0) { slots2[idx2]={...slots2[idx2],name:"",price:"",done:false,recurWeeks:null,isException:false}; setSlots(entry.dateKey,slots2); }
    } else if (entry.type === "removed") {
      var slots3 = [...getSlots(entry.dateKey)];
      var idx3 = slots3.findIndex(function(s){ return s.time===entry.time; });
      if (idx3>=0) { slots3[idx3]={...slots3[idx3],name:entry.name,price:entry.price||"",done:false}; setSlots(entry.dateKey,slots3); }
    } else if (entry.type === "edited") {
      var slots4 = [...getSlots(entry.dateKey)];
      var idx4 = slots4.findIndex(function(s){ return s.time===entry.time; });
      if (idx4>=0) { slots4[idx4]={...slots4[idx4],name:entry.prevName}; setSlots(entry.dateKey,slots4); }
      ...
      Object.keys(newSch).forEach(function(dk) {
        ...
        var dSlots = [...newSch[dk]];
        var si = dSlots.findIndex(function(s){ return s.time===entry.time&&s.name===entry.name&&s.recurWeeks===entry.weeks; });
        if (si>=0) {
  ...
  const handleKeyDown = function(e, dateKey, idx) {
    ...
    if (e.key==="Enter" && e.shiftKey) {
      e.preventDefault();
      var currentVals = editValuesRef.current;
      var slots = [...getSlots(dateKey)];
      var currentSlot = slots[idx];
      var newName = capitalizeFirst((currentVals.name||"").trim());
      var newPrice = (currentVals.price||"").trim();
      if (!newName) return;
      var gid = currentSlot.groupId || (idx > 0 && slots[idx-1].groupId) || newGroupId();
      ...
      var s=getSlots(dateKey);
      if(idx<s.length-1) setTimeout(function(){ startEdit(dateKey,idx+1); },80);
    } else if (e.key==="ArrowUp") {
      e.preventDefault();
      doCommit(dateKey,idx,editValuesRef.current);
      if(idx>0) setTimeout(function(){ startEdit(dateKey,idx-1); },80);
    } else if (e.key==="Escape") { editingRef.current=null; setEditingCell(null); setEditingOccupied(false); }
  };

  const getNextDateKey = function(fromDateKey, weeks) {
    var next = addWeeks(parseDateKey(fromDateKey), weeks);
  ...
  const confirmNextBooking = function(targetDateKey) {
    ...
    if (slot.recurWeeks) {
      ...
      var cursor = parseDateKey(targetDateKey);
      while (true) {
        cursor = addWeeks(cursor, slot.recurWeeks);
        if (cursor > sixMonthsOut) break;
        var futureKey = formatDateKey(cursor);
        var fSlots = newSchedules[futureKey]
        ...
        if (fi>=0) {
          ...
          fSlots.sort(function(a,b){ return parseTime(a.time)-parseTime(b.time); });
        }
  ...
  const openClientProfile = function(name) {
    ...
    var nonException = bookings.filter(function(b){ return !b.isException; });
    var usualTime = nonException.length>0 ? nonException[0].time : (bookings[0] && bookings[0].time) || "";
    var recurWeeks = bookings.find(function(b){ return b.recurWeeks; }) ? bookings.find(function(b){ return b.recurWeeks; }).recurWeeks : null;
  ...
  const startLongPress = function(name) {
  ...
  const handleReassignSlotTap = function(dateKey, idx) {
    ...
    var client = reassignMode.client;
    var remainingConflicts = reassignMode.remainingConflicts;
    var slots = [...getSlots(dateKey)];
    var slot = slots[idx];
    if (slot.name) return;
    var newSlots = [...slots];
    ...
    addHistoryEntry({type:"added", time:slot.time, name:client.name, price:client.price, dateKey, note:"reassigned"});
    if (reassignMode.originalDateKey && reassignMode.originalIdx !== undefined) {
      ...
      addHistoryEntry({type:"removed", time:origSlot.time, name:client.name, dateKey:reassignMode.originalDateKey, note:"moved"});
    }
  ...
  const applyAltTimeToConflicts = function(altTime, conflicts, client) {
    ...
    conflicts.forEach(function(c) {
      var daySlots = newSchedules[c.dateKey]
        ? [...newSchedules[c.dateKey]]
        : DEFAULT_TIMES.map(function(t){ return {time:t,name:"",price:"",done:false,recurWeeks:null}; });
      var targetIdx = daySlots.findIndex(function(s){ return s.time===altTime; });
  ...
  const buildRecurringSchedules = function(baseSchedules, dateKey, sourceSlot, weeks) {
    var newSchedules = {...baseSchedules};
    var conflicts = [];
    var sixMonthsOut = new Date();
    sixMonthsOut.setMonth(sixMonthsOut.getMonth() + 6);
    ...
    while (true) {
      ...
      if (cursor > sixMonthsOut) break;
      var futureKey = formatDateKey(cursor);
      var daySlots = newSchedules[futureKey]
        ? [...newSchedules[futureKey]]
        : DEFAULT_TIMES.map(function(t){ return {time:t,name:"",price:"",done:false,recurWeeks:null}; });
      var existingIdx = daySlots.findIndex(function(s){ return s.time===sourceSlot.time; });
  ...
  const setRecurring = function(dateKey, idx, weeks) {
    var sourceSlots = [...getSlots(dateKey)];
    var sourceSlot = sourceSlots[idx];
    sourceSlots[idx] = {...sourceSlot, recurWeeks: weeks};
    var baseSchedules = {...schedulesRef.current, [dateKey]: sourceSlots};
    if (weeks) {
      var result = buildRecurringSchedules(baseSchedules, dateKey, sourceSlot, weeks);
      var newSchedules = result.newSchedules; var conflicts = result.conflicts;
      ...
      var newSch2 = {...schedulesRef.current, [dateKey]: sourceSlots};
      var oldWeeks = sourceSlot.recurWeeks;
      if (oldWeeks) {
        var sixMo2 = new Date(); sixMo2.setMonth(sixMo2.getMonth()+6);
        var cur2 = parseDateKey(dateKey);
        while (true) {
          cur2 = addWeeks(cur2, oldWeeks);
          if (cur2 > sixMo2) break;
          var fk2 = formatDateKey(cur2);
          if (newSch2[fk2]) {
            var ds2 = [...newSch2[fk2]];
            var si2 = ds2.findIndex(function(s){ return s.time===sourceSlot.time&&s.name===sourceSlot.name&&s.recurWeeks===oldWeeks&&!s.done; });
            if (si2>=0) { ds2[si2]={...ds2[si2],name:"",price:"",recurWeeks:null,done:false}; newSch2[fk2]=ds2; }
          }
        }
      }
      setSchedules(newSch2);
      addHistoryEntry({type:"recurring_set",time:sourceSlot.time,name:sourceSlot.name,weeks,dateKey});
      setRecurringModal(null);
    }
  };
  ...
  const requestRemoveSlot = function(dateKey, idx) {
    ...
    if (slot.groupId) {
      var slots = getSlots(dateKey);
      var groupSlots = slots.filter(function(s){ return s.groupId===slot.groupId&&s.name; });
  ...
  const cancelGroupSlots = function(dateKey, groupId, onlyIdx) {
    ...
    if (onlyIdx !== undefined) {
      var slot = slots[onlyIdx];
      slots[onlyIdx] = {...slot, name:"", price:"", done:false, recurWeeks:null, isException:false, groupId:null};
      var remaining = slots.filter(function(s){ return s.groupId===groupId&&s.name; });
      if (remaining.length === 1) {
        var ri = slots.findIndex(function(s){ return s.groupId===groupId&&s.name; });
  ...
  const rescheduleGroupSlots = function(dateKey, groupId, onlyIdx) {
    var targetSlot = getSlots(dateKey)[onlyIdx];
    setReassignMode({
      ...
      <div style={{marginTop:"4px"}}>
        ...
        <div style={{display:"grid",gridTemplateColumns:"repeat(7,1fr)",gap:"3px"}}>
          {cells.map(function(day, i) {
            ...
            var daySlots = getSlots(dk);
            var bookedSlots = daySlots.filter(function(s){ return s.name; });
            var slotTakenByOther = daySlots.some(function(s){ return s.time===slot.time && s.name && s.name.toLowerCase()!==slot.name.toLowerCase(); });
            var isT = isToday(day);
  ...
  return (
    <div style={{minHeight:"100vh",background:"#ffffff",fontFamily:"Georgia,serif",color:"#1a1a1a",paddingTop:reassignMode?"52px":"0"}}
        ...
        <div style={{position:"fixed",inset:0,background:"rgba(0,0,0,0.6)",zIndex:1100,display:"flex",alignItems:"center",justifyContent:"center"}}>
          <div style={{background:"#fff",border:"1px solid #e0e0de",borderRadius:"12px",padding:"24px",width:"min(320px,92vw)"}}>
            ...
            <button onClick={function(){
              ...
              else rescheduleGroupSlots(groupConfirm.dateKey,groupConfirm.groupId,groupConfirm.idx);
            }} style={{display:"block",width:"100%",padding:"11px",background:"#1a1a1a",border:"none",borderRadius:"8px",color:"#fff",cursor:"pointer",fontFamily:"inherit",fontSize:"13px",marginBottom:"8px"}}>Just this one slot</button>
            <button onClick={function(){
              if(groupConfirm.action==="cancel") cancelGroupSlots(groupConfirm.dateKey,groupConfirm.groupId,undefined);
              else rescheduleGroupSlots(groupConfirm.dateKey,groupConfirm.groupId,undefined);
            }} style={{display:"block",width:"100%",padding:"11px",background:"#fff",border:"1px solid #d8d8d6",borderRadius:"8px",color:"#666",cursor:"pointer",fontFamily:"inherit",fontSize:"13px",marginBottom:"8px"}}>All of {groupConfirm.name}'s slots</button>
                ...
                {groupRecurModal.weeks && (
                  <button onClick={function(){
                    ...
                    if(gd.recurCount===1){
                      ...
                      gd.groupSlots.slice(0,gd.recurCount).forEach(function(gs){ setRecurring(gd.dateKey,gs.i,gd.weeks); });
                      setGroupRecurModal(null);
                    }
      ...
      {conflictModal && (
        <div style={{position:"fixed",inset:0,background:"rgba(0,0,0,0.88)",zIndex:1100,display:"flex",alignItems:"center",justifyContent:"center"}}>
          <div style={{background:"#ffffff",border:"1px solid #e0e0de",borderRadius:"12px",padding:"28px 28px 24px",width:"min(400px,92vw)",maxHeight:"80vh",overflowY:"auto"}}>
            ...
            <div style={{fontSize:"12px",color:"#888",marginBottom:"16px"}}>{conflictModal.conflicts[0] && conflictModal.conflicts[0].name} will be placed on all open dates. The following dates need your attention.</div>
            <div style={{marginBottom:"20px"}}>
              {conflictModal.conflicts.map(function(c,i){ return (
                <div key={i} style={{display:"flex",alignItems:"center",justifyContent:"space-between",padding:"9px 10px",marginBottom:"4px",background:"#fff5f4",border:"1px solid #f0d0cc",borderRadius:"6px"}}>
                  ...
                  <button onClick={function(){
                    var remaining = conflictModal.conflicts.filter(function(_,j){ return j!==i; });
                  ...
                  }} style={{padding:"6px 12px",background:"#1a1a1a",border:"none",borderRadius:"6px",color:"#fff",cursor:"pointer",fontFamily:"inherit",fontSize:"11px",flexShrink:0,marginLeft:"10px"}}>Jump</button>
                </div>
              ); })}
            </div>
        ...
        <div style={{position:"fixed",inset:0,background:"rgba(0,0,0,0.88)",zIndex:1000,display:"flex",alignItems:"center",justifyContent:"center"}}>
          <div style={{background:"#f8f8f6",border:"1px solid #d8d8d6",borderRadius:"12px",padding:"28px 28px 24px",width:"min(340px,92vw)"}}>
            ...
            {recurringModal.slot.recurWeeks && (
              <button onClick={function(){ setRecurring(recurringModal.dateKey,recurringModal.idx,null); }} style={{display:"block",width:"100%",padding:"8px",background:"none",border:"1px solid #d8d8d6",borderRadius:"6px",color:"#999",cursor:"pointer",fontFamily:"inherit",fontSize:"11px",letterSpacing:"0.1em",textTransform:"uppercase",marginBottom:"12px"}}>Remove recurring</button>
            )}
            ...
            {checkoffModal.notRecurring ? (
              <div>
                ...
                <div style={{display:"flex",gap:"8px",flexWrap:"wrap",marginBottom:"20px"}}>
                  {[2,3,4,5,6].map(function(w){
                    var d = addWeeks(parseDateKey(checkoffModal.dateKey), w);
                    var dk = toDateKey(d);
                    var mo = d.getMonth();
                    var dateStr = [3,4,5,6].includes(mo) ? d.toLocaleDateString("en-US",{month:"long",day:"numeric"}) : d.toLocaleDateString("en-US",{month:"short",day:"numeric"});
                    return (
                      <button key={w} onClick={function(){ jumpToDateForBooking(dk, checkoffModal.slot); }} style={{padding:"9px 16px",background:"#f4f4f2",border:"1px solid #d8d8d6",borderRadius:"8px",cursor:"pointer",fontFamily:"inherit",fontSize:"13px",color:"#1a1a1a"}}>
                        {w}w · {dateStr}
                      </button>
                    );
                  })}
              ...
              <div>
                ...
                {nudgedDate && nudgedDate !== checkoffModal.nextDateKey && (
                  <div style={{fontSize:"11px",color:"#a07830",marginBottom:"10px"}}>Nudged — schedule resumes every {checkoffModal.slot.recurWeeks===1?"week":(checkoffModal.slot.recurWeeks+" weeks")} after this</div>
                )}
                <div style={{display:"flex",gap:"8px",marginBottom:"20px"}}>
                  <button onClick={function(){ confirmNextBooking(effectiveNextDate); }} style={{flex:1,padding:"12px",background:nudgeConflict?"#5a2a1a":"#c9a96e",border:"none",borderRadius:"8px",color:nudgeConflict?"#e8b84b":"#0f0f0f",cursor:"pointer",fontFamily:"inherit",fontSize:"13px"}}>
      ...
      <div style={{borderBottom:"1px solid #e8e8e6",padding:"18px 20px 14px",display:"flex",alignItems:"center",justifyContent:"space-between",position:"sticky",top:0,background:"#ffffff",zIndex:100}}>
        <div style={{display:"flex",gap:"2px",background:"#e8e8e6",padding:"3px",borderRadius:"6px"}}>
          {VIEWS.map(function(v){ return (
            <button key={v} onClick={function(){
              if(v==="Wknd") setBaseDate(getUpcomingWeekend());
              setView(v);
        ...
        {view==="Month" && (
          <div style={{fontSize:"14px",color:"#1a1a1a",letterSpacing:"0.01em"}}>{baseDate.toLocaleDateString("en-US",{month:"long",year:"numeric"})}</div>
        )}
        <div style={{display:"flex",gap:"4px",alignItems:"center"}}>
          ...
          <button onClick={function(){
            if(view==="Month"){var d=new Date(baseDate);d.setMonth(d.getMonth()-1);setBaseDate(d);}
            else setBaseDate(function(d){ return addDays(d,-1); });
          }} style={navBtn}>{"‹"}</button>
          <button onClick={function(){
            if(view==="Wknd") setBaseDate(getUpcomingWeekend());
            else setBaseDate(new Date());
          }} style={{...navBtn,fontSize:"9px",letterSpacing:"0.1em",padding:"0 12px"}}>TODAY</button>
          <button onClick={function(){
            if(view==="Month"){var d=new Date(baseDate);d.setMonth(d.getMonth()+1);setBaseDate(d);}
            else setBaseDate(function(d){ return addDays(d,1); });
          }} style={navBtn}>{"›"}</button>
            ...
            return (
              <div key={dateKey} style={{background:"#ffffff",display:"flex",flexDirection:"column"}}>
                <div style={{padding:"12px 14px 10px",borderBottom:"1px solid #ebebea",display:"flex",justifyContent:"space-between",alignItems:"flex-end"}}>
                  <div>
                    {(function(){
                      var sz = view==="Day"?"22px":"16px";
                      var mo = date.getMonth();
                      var monthStr = [3,4,5,6].includes(mo)
                ...
                <div style={{flex:1,padding:"6px 0"}}>
                  {slots.map(function(slot,idx){
                    ...
                    var filled = !!slot.name;
                    var wasRemoved = recentlyRemoved[(dateKey+"-"+idx)];
                    var isSwiped = swipedSlot===(dateKey+"-"+idx);
                    var rowKey = (dateKey+"-"+idx);
                    var isOccupiedEdit = isEditing && editingOccupied;
                    var slotBg = slot.blocked?"#f4f4f2":wasRemoved?"#fff0ee":isOccupiedEdit?"#fff0ee":slot.done?"#f4faf4":isEditing?"#f0f0ee":filled?"#fcfcfa":"transparent";
                    return (
                      <div key={rowKey} style={{position:"relative",overflow:"hidden",borderBottom:"1px solid #efefed"}}>
                        {filled && (
                          <div style={{position:"absolute",right:0,top:0,bottom:0,width:"160px",display:"flex",alignItems:"stretch",opacity:isSwiped?1:0,pointerEvents:isSwiped?"auto":"none",transition:"opacity 0.2s"}}>
                            <button onClick={function(){
                              setSwipedSlot(null);
                              if(slot.groupId){var gs=getSlots(dateKey).filter(function(s){ return s.groupId===slot.groupId&&s.name; });if(gs.length>1){setGroupConfirm({action:'reschedule',dateKey,idx,name:slot.name,groupId:slot.groupId});return;}}
                              setReassignMode({client:{name:slot.name,price:slot.price,recurWeeks:slot.recurWeeks},currentDateKey:dateKey,originalDateKey:dateKey,originalIdx:idx,remainingConflicts:[]});
                              setView("Day");
                        ...
                        <div
                          style={{display:"flex",alignItems:"center",padding:"0 14px",height:"46px",background:slotBg,transition:"transform 0.2s, background 0.3s",transform:isSwiped?"translateX(-160px)":"translateX(0)",position:"relative",opacity:slot.blocked?0.6:1}}
                          onTouchStart={function(e){ handleTouchStart(e,dateKey,idx); }}
                          onTouchEnd={function(e){ handleTouchEnd(e,dateKey,idx); }}
                        >
                          {(wasRemoved||isOccupiedEdit)&&<div style={{position:"absolute",left:0,top:0,bottom:0,width:"3px",background:"#c0392b"}}/>}
                          {slot.groupId&&!wasRemoved&&(function(){
                            var daySlots = getSlots(dateKey);
                            var gSlots = daySlots.map(function(s,i){ return {...s,i}; }).filter(function(s){ return s.groupId===slot.groupId&&s.name; });
                            var first = gSlots[0] && gSlots[0].i === idx;
                            var last = gSlots[gSlots.length-1] && gSlots[gSlots.length-1].i === idx;
                            var inGroup = gSlots.some(function(s){ return s.i===idx; });
                          ...
                          {slot.blocked ? (
                            ...
                            <div style={{flex:1,display:"flex",alignItems:"center",gap:"4px"}}>
                              {isOccupiedEdit && (
                                <div style={{position:"absolute",top:"2px",left:"70px",fontSize:"9px",color:"#c0392b"}}>Editing {slot.name}</div>
                              )}
                              <input
                                ...
                                <div style={{display:"flex",alignItems:"center",gap:"6px",flexShrink:0}}>
                                  ...
                                  {filled&&(
                                    <button onClick={function(e){
                                      e.stopPropagation();
                                      if(slot.groupId){var allSlots=getSlots(dateKey);var gSlots=allSlots.map(function(s,i){ return {...s,i}; }).filter(function(s){ return s.groupId===slot.groupId&&s.name; });if(gSlots.length>1){setGroupRecurModal({dateKey,idx,slot,groupSlots:gSlots,weeks:null});return;}}
                  ...
                  <div style={{display:"flex",gap:"6px",padding:"10px 14px"}}>
                    <button onClick={function(){ addSlotToBeginning(dateKey); }} style={{flex:1,padding:"9px",background:"#f4f4f2",border:"1px solid #d8d8d6",borderRadius:"6px",color:"#888",cursor:"pointer",fontFamily:"inherit",fontSize:"11px",letterSpacing:"0.08em"}}
                      onMouseEnter={function(e){ e.currentTarget.style.background="#e8e8e6"; }}
                      onMouseLeave={function(e){ e.currentTarget.style.background="#f4f4f2"; }}
                    >+ AM</button>
                    <button onClick={function(){ addSlotToEnd(dateKey); }} style={{flex:1,padding:"9px",background:"#f4f4f2",border:"1px solid #d8d8d6",borderRadius:"6px",color:"#888",cursor:"pointer",fontFamily:"inherit",fontSize:"11px",letterSpacing:"0.08em"}}
                      onMouseEnter={function(e){ e.currentTarget.style.background="#e8e8e6"; }}
                      onMouseLeave={function(e){ e.currentTarget.style.background="#f4f4f2"; }}
                    >+ PM</button>
                  </div>
